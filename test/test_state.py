...
def test_allocate_returns_init_order_then_caller_drives_state_machine():
    state = init_state()
...
def test_filter_rejection_in_allocate_returns_empty_list_no_state_change():
    """A ticket whose notional is below the minimum is rejected by the
    symbol's filter chain inside allocate. Allocate treats per-bucket
    filter rejection as best-effort: the bucket is skipped, the
    surviving result is returned. With a single (primary) bucket here,
    the surviving result is the empty list and state is never
    modified."""
    state = init_state()
...
def test_purge_fully_settled_after_balance_catches_up_with_commission():
    ...
    # Use BTCUSDT (primary-quote) so allocate produces a single order
    # on the primary bucket and the symbol on the resulting Order
    # matches what the balance updates below reflect.
    ticket = _make_buy_limit_ticket(
    ...
    state.set_balances([
        Balance(USDT, Decimal('190000'), Decimal('0'), t_settle),
    ])
    state.set_balances([
